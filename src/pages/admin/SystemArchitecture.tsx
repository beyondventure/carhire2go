        ...
        <div className="hidden print:block text-center py-16">
          <div className="space-y-8">
            <div className="space-y-4">
              <h1 className="text-5xl font-bold text-primary">CarHire2Go</h1>
              <p className="text-2xl text-muted-foreground">System Architecture Document</p>
            </div>
            <div className="w-32 h-1 bg-gradient-to-r from-primary via-accent to-primary mx-auto rounded-full" />
            <div className="space-y-2 text-lg">
              <p className="font-semibold">Enterprise Vehicle Hire Platform</p>
              <p className="text-muted-foreground">Version 1.0 | January 2026</p>
            </div>
            <div className="pt-12 text-sm text-muted-foreground">
              <p>Confidential Technical Documentation</p>
              <p>For Client Review Only</p>
            </div>
          </div>
        </div>

        <Tabs value={activeSection} onValueChange={setActiveSection} className="space-y-6">
          <TabsList className="grid grid-cols-6 w-full no-print">
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {[
                  ...
                  { name: 'ChatPanel', desc: 'Real-time messaging for negotiations' },
                ].map((comp) => (
                  <div key={comp.name} className="bg-muted/50 rounded-lg p-3">
                    <code className="text-sm font-mono text-primary">{comp.name}</code>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                {[
                  { name: 'useSupabaseAuth', desc: 'Authentication state and methods' },
                  { name: 'useBookings', desc: 'Booking CRUD operations and real-time' },
                  { name: 'useProviders', desc: 'Provider data management' },
                  { name: 'useDrivers', desc: 'Driver state and operations' },
                  { name: 'useVehicles', desc: 'Fleet management operations' },
                ].map((hook) => (
                  <div key={hook.name} className="bg-muted/50 rounded-lg p-3">
                    <code className="text-sm font-mono text-accent">{hook.name}</code>
...
function DatabaseSection() {
  const tables = [
    {
      name: 'profiles',
      desc: 'User profile information and preferences',
      columns: ['id (UUID, PK)', 'name (TEXT)', 'email (TEXT)', 'phone (TEXT)', 'avatar_url (TEXT)', 'verified (BOOLEAN)', 'created_at (TIMESTAMP)'],
      relations: 'References auth.users(id)',
    },
    {
      name: 'user_roles',
      desc: 'Role assignments for multi-role support',
      columns: ['id (UUID, PK)', 'user_id (UUID, FK)', 'role (app_role ENUM)'],
      relations: 'References profiles(id)',
    },
    {
      name: 'providers',
      desc: 'Car hire company information',
      columns: ['id (UUID, PK)', 'user_id (UUID, FK)', 'business_name (TEXT)', 'business_address (TEXT)', 'provider_type (ENUM)', 'service_areas (TEXT[])', 'verified (BOOLEAN)', 'rating (DECIMAL)', 'acceptance_rate (DECIMAL)'],
      relations: 'References profiles(id)',
    },
    {
      name: 'vehicles',
        ...
        <CardContent>
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              { name: 'POPIA', desc: 'SA Privacy Act' },
